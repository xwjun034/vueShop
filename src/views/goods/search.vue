<template>
  <div class="page">
    ...
    <my-search :show="searchShow" :isLocal="true" />
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      keyword: this.$route.query.keyword ? this.$route.query.keyword : "", // 接受从搜索组件传递的参数
  ...
  methods: {
    ...
    selectPriceOrder(index) {
      if (this.priceOrderList.length > 0) {
        for (var i = 0; i < this.priceOrderList.length; i++) {
    ...
    getSearchData(){
      axios({
        url:URL.getPrice+'home/goods/search?kwords='+this.searchJson.kwords+'&param='+this.searchJson.param+'&page='+this.searchJson.page+'&price1='+this.searchJson.price1+'&price2='+this.searchJson.price2+'&otype='+this.searchJson.otype+'&cid='+this.searchJson.cid+'&token=1ec949a15fb709370f',
        method:'GET'
      }).then((res)=>{
