...
export default {
  ...
  methods: {
    ...
    goSearch(keyword) {
      ...
      this.show.show=false; // 关闭search组件
      //判断是否是本地址
      if(this.isLocal){
        this.$router.replace('/goods/search?keyword='+tmpKeyword); // 跳转搜索到的数据页面 传参数到搜索的页面
      }else{
        this.$router.push('/goods/search?keyword='+tmpKeyword); // 跳转搜索到的数据页面 传参数到搜索的页面
      }
